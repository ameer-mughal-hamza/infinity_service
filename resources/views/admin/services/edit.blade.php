                ...
                <div id="content">
                    ...
                    <div class="content-inner">
                        <div class="form-wrapper">
                            <form method="POST" action="{{ route('update_service') }}" role="form">
                                ...
                                <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                                    ...
                                    <textarea class="form-control" name="description" id="description"
                                              ...
                                              rows="10">{{ $service->description }}{{ old('description') }}</textarea>
                                ...
                                <div class="form-group">
                                    <select data-placeholder="Select tags" class="form-control chosen-select" multiple
                                        ...
                                        @foreach($sub_services as $key => $sub_service)
                                            @if($key < count($service->subservices))
                                                <option value="{{ $sub_service->id }}"{{ $service->subservices[$key]->title == $sub_service->title ? 'selected' : '' }}>{{ $sub_service->title }}</option>
                                            @else
